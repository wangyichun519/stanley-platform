...
async function generatePostContent(dayOfWeek) {
  ...
  if (!apiKey) {
    console.log('[Threads] 未設定 GROQ_API_KEY，使用靜態備用文');
    return fallback;
  }
    ...
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `你是 Stanley 的 Threads 社群小編，正在籌備一個結合精品咖啡、銀髮長照與漢方養生的品牌，品牌名稱為「Coffee Priority」，基地在花蓮。目前品牌仍在籌備階段，尚未正式成立公司或開業，所以發文語氣是「個人分享知識與理念」，而不是「公司宣傳」。絕對不可以用「歡迎來電」「預約諮詢」「立即購買」「目前開課」「名額有限」等已開業的說法。語氣：真誠、有溫度、像在跟朋友聊天，偶爾帶點故事感。只輸出貼文本文，不加任何說明或引號。全文必須使用正體中文（繁體中文），絕對不可以出現任何簡體字。`
          },
          {
            role: 'user',
...
8. 只輸出貼文，不加說明`
    ...
    if (!res.ok) {
      ...
      console.error('[Threads] Groq API 錯誤:', err);
      return fallback;
    }
